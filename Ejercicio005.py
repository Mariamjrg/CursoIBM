#Ejercicio5
#Escribí un programa que solicite al usuario el ingreso de una 
# temperatura en escala Fahrenheit (debe permitir decimales) 

Fahrenheit=float(input('temperatura en escala fahrenheit:'))
#y le # muestre el equivalente en grados Celsius. La fórmula de conversión 
# que se usa para este cálculo es: # _Celsius = (5/9) * (Fahrenheit-32)_

print('equivalente grados Celsius:',(5/9) * (Fahrenheit-32))
